...
test("o livro apresenta o arco completo sem fingir páginas ainda não escritas", async ({ page }) => {
  ...
  await expect(page.getByText("063")).toBeVisible();
  await expect(page.getByText(/páginas publicadas de 216/i)).toBeVisible();
  await expect(page.getByRole("link", { name: "Ler capítulo" })).toHaveCount(7);
  await expect(page.getByText(/Planejado · p. 64–72/)).toBeVisible();
});
